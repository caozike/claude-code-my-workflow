...
7. **Keep the surfaces in sync** when adding features. Adding a skill means **adding its row to the README `<!-- surface-sync-table: skills -->` table** *and* keeping the prose counts (e.g. "50 skills") in sync across `README.md`, `docs/index.html`, the guide, and `templates/skill-template.md`. `./scripts/check-surface-sync.sh` enforces **both** the counts and the table rows — run it before you open a PR.
